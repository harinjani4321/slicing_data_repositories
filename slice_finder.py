...
"""
Slice is expressed as a dictionary.
Continuous feature "a <= c <= b" is represented as {'c':[[a, b]]}
Discrete feature "d = a" is represented as {'d':[['a']]}
"""
...
class SliceFinder2:
    def __init__(self, features, ground, predicted):
        self.features = features
        self.ground = ground
        self.predicted = predicted

    def find_slice_2(self, k=3, epsilon=0.2, alpha=0.05, degree=3,
                     risk_control=True, max_workers=1):
        assert k > 0, 'Number of recommendation k should be greater than 0'

        metrics_all = self.evaluate_model_2(self.ground, self.predicted)
        ...
        for i in range(1, degree + 1):
            if i == 1:
                candidates = self.slicing_2()
            else:
                candidates = self.crossing_2(uninteresting, i)
            interesting, uninteresting_ \
                = self.filter_by_effect_size_2(candidates, reference, epsilon,
    ...
    def evaluate_model_2(self, ground, predicted, metric=log_loss):
        y_p = np.zeros([np.size(predicted), 2])
        y_p[:, 0] = predicted[:]
        y_p[:, 1] = 1 - predicted[:]
        y_p = list(map(functools.partial(np.expand_dims, axis=0), y_p))
        y = list(map(functools.partial(np.expand_dims, axis=0), ground))
        if metric == log_loss:
            return list(map(functools.partial(metric, labels=[0, 1]), y, y_p))
        elif metric == accuracy_score:
            return list(map(metric, y, y_p))

    def filter_by_effect_size_2(self, slices, reference,
                                epsilon=0.5, max_workers=1, alpha=0.05,
                                risk_control=True):
        filtered_slices = []
        rejected = []
        ...
        for s in slices:
            if s.size == 0:
                continue
            s = self.eff_size_job_2(s, reference, alpha)
        ...
        #         if s.size == 0:
        #             continue
        #         batch_jobs.append(executor.submit(self.eff_size_job_2,
        #                                           s, reference, alpha))
    ...
    def slicing_2(self):
        ...
        for col in X.columns:
            ...
            if len(uniques) == n:
                continue
            if len(uniques) > n / 2.:
                bin_edges = self.binning_2(X[col], n_bin=10)
    ...
    def crossing_2(self, slices, degree):
        crossed_slices = []
    ...
    def binning_2(self, col, n_bin=20):
        bin_edges = stats.mstats.mquantiles(col, np.arange(0., 1. + 1. / n_bin, 1. / n_bin))
        return bin_edges

    def eff_size_job_2(self, s, reference, alpha=0.05):
        m_slice = self.evaluate_model_2(self.ground.loc[s.data_idx],
                                        self.predicted.loc[s.data_idx])
